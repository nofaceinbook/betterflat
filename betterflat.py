...
def printInfo():
    print ("------------------------------------------------------------------------------")
    print ("             X-PLANE betterflat   version 0.1 (test)  by schmax")
    print ("This script adapts an X-Plane dsf-file to flatten height at an airport.")
    print ("Usage: betterflat <dsf-filename> <apt.dat of airport> <opt: new heigth in m>")
    ...
    print ("The new heigt is either given as optional third argument in meter or")
...
def readBoundaryFromAPT(filename):
#
...
#dsffile = os.fspath(r'D:\Programmierung\DSF_Reader\+50-126.dsf')
#aptfile = os.fspath(r'D:\Programmierung\DSF_Reader\apt.dat')

print ("Reading", aptfile, "to extract airport boundary.")
poly = readBoundaryFromAPT(aptfile) ## Require r befor string to not get ASCII bell with \a !!!!!!!!!!!!!!
print ("Extracted boundary with", len(poly), "vertices.")

dsf = XPLNEDSF()
if dsf.read(dsffile): #returns value > 0 in case of errors
    sys.exit(1)

print("Reading of dsf-file completed. Checking now for mesh triangles intersecting airport boundary.......")
