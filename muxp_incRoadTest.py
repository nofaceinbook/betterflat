...
class muxpGUI:
    ...
    def runMuxp(self, filename):
        ...
        log.info("Properties: {}".format(self.dsf.Properties))
        self.dsf.Properties["muxp/dsf_basehash"] = "mmmsjfsdfkjsdljfsdlkfjxxx"
        log.info("Network: {}".format(self.dsf.Networks))
        log.info("Vertices of Network:")
        for i in range(10):
            log.info("   {}".format(self.dsf.V32[0][i]))
        log.info("################### Network Chunks in Poly ######################")
        pol = [[-14.406682, -7.950752], [-14.406489, -7.950606], [-14.406222, -7.950900], [-14.406366, -7.951115], [-14.406682, -7.950752]]
        for c in self.dsf.getChains():
            for v in c:
                if PointInPoly((self.dsf.V32[v[0]][v[1]][0], self.dsf.V32[v[0]][v[1]][1]), pol):
                    log.info("{}".format(c))
                    for x in c:
                        log.info("    {}".format(self.dsf.V32[x[0]][x[1]]))
        self.dsf.V32[0][323][2] = 2.0
        self.dsf.V32[0][324][2] = 5.0
        self.dsf.V32[0][325][2] = 8.0
        self.dsf.V32[0][838][2] = 8.0
        self.dsf.V32[0][837][2] = 2.0
        #

        log.info("Properties: {}".format(self.dsf.Properties))
        for i in range(10):
            log.info("  V32: {}".format(self.dsf.V32[0][i]))
        log.info("Elevation Junction at 325:   {}".format(self.dsf.V32[0][325]))
        log.info("Elevation Junction at 837:   {}".format(self.dsf.V32[0][837]))

        a = muxpArea(self.dsf, LogName)
        a.extractMeshArea(*update["area"])
        kmlExport2(self.dsf, [[]], a.atrias, "D:\\Programmierung\\muxp_0_1\\test_extract_before_Change.dsf")
        ...
        commandbound1 = []
        for c in update["commands"][0]["coordinates"]:
            commandbound1.append(c)
        commandbound1.append(update["commands"][0]["coordinates"][0] ) #to get closed poly
        ...
        #log.info("Poly returned from cut: {}".format(new_polys))
        #exit(0)
        for t in a.atrias:
            #if isPointInTria([-14.402184, -7.967070], t):
            if isPointInTria([-14.405320, -7.969124], t):
                tria = [t[0][0:2], t[1][0:2], t[2][0:2]]
        #log.info("Cutting tria: {}".format(tria))
        polysouter, polysinner, borderv = a.CutPoly(commandbound1)
        log.info("Poly returned from cut: {}".format(polysouter))
        log.info("Border returned from cut: {}".format(borderv))
        new_polys = polysinner
        new_polys[0].append(new_polys[0][0])
        new_polys.append(commandbound1)
        kmlExport2(self.dsf, new_polys, a.atrias, "D:\\Programmierung\\muxp_0_1\\test_extract.dsf")
        exit(0) ###################### END HERE FOR TESTING #################################
        a.createDSFVertices()
        a.insertMeshArea()
