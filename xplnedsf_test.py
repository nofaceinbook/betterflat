...
def PointInPolyDSF(p, poly):  # test wether a point p with [lat, lon] coordinates lies in polygon (list of [lat, lon] pairs        ######## TAKEN FROM bflat.py ##############
 ...
 ########### END of NEW function _packCMDS_() #################               
    ...
    def insertVertex(self, v, accuracy=10): #adds new vertex to dsf and updates mesh accordingly, returns exsting verticy when it is closer than accuracy in m, returns tuple of Pool, Index as set   ########### NEW ###########
        ...
        for p in l: #now go only through potential patches to check details
                ...
                if isPointInTria(v, tv):
                    ...
                    for e in range(3): #go through edges of tria and check if vertex v is too close to one of them
                        ...
                        if distance(v, closestPoint) < accuracy:  ###### TBD: accurcy as variable of function !!!! ######
                            ...
                            return self.cutEdges(v, [v[0] - lo * 1.000001 * (ortho[0] - tv[e][0]),  v[1] - lo * 1.000001 * (ortho[1] - v[1])], accuracy) #  * 1.000001 to really cross edge with ortho vector in - direction
                    #### TBD: Make sure Pool Index does not exceed/reach 2^16 --> Could be handled when pools are converted to binary
            ...
            p.trias2cmds() ##Update Commands accordingly
        return [(v_Pool, v_Index)]
    
    def cutPolyInMesh(self, poly, accuracy=10): #updates dsf mesh to contain the polygon shape as edges (if existing vertices, edges are closer than accourcy in m they will be used), elevation will be given by mesh
        ...
        for i in range(len(poly) - 1):   # assumes that last vertex in poly is same as first, to have a closed polygon
            ...
            for v in self.insertVertex(poly[i], accuracy): ### SHOULD ACTUALLB BE ADDED DIRECTLY AFTER EDGE
                border_vertices.add(v)
        ####### PROBLEM: THIS VERSION IS STILL CUTTING EDGES, EVEN IF CLOSE TO EXISTING VERTEX !!!! ##############################
        ########### AFTER CHANGING INDENT OF else: in CutEdges above, even worse; now some eges aren't cut any more !!!!!! #############
        print(border_vertices)
        print("These are", len(border_vertices), "vertices at the border!")
        inner_vertices = set()
        ...
        for p in l:
            for t in p.trias:
                ...
                for i in range(3):
                    if PointInPolyDSF(vt[i], poly):
                        inner_vertices.add((t[i][0], t[i][1]))
        print(inner_vertices)
        print("These are", len(inner_vertices), "inner vertices to be flattened!")
        print("-------------- All vertices flattended ----------------")
        print(border_vertices.union(inner_vertices))
        kmlExport(self, [poly], border_vertices.union(inner_vertices), "newbflat.kml")
        
        ### Identify all vertices inside poly + add to set
        ### Adapt height and then normal vectors (if required when no raster) for all vertices in set
        ### All trias that are only build by vertices in set are the one building the flat Poly and can be used to show in .kml-export
